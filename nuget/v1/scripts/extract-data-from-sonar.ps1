param ($sqprojectname,$sonarqubeThreshold)
...
Write-Host " auth header $basicAuth"
...
$bugsUrl = "https://sonar.transunion.com/api/issues/search?componentKeys=" + $sqprojectname + "&s=FILE_LINE&resolved=false&sinceLeakPeriod=true&types=BUG&ps=100&organization=default-organization&facets=severities%2Ctypes&additionalFields=_all"
$codeSmellsUrl = "https://sonar.transunion.com/api/issues/search?componentKeys=" + $sqprojectname + "&s=FILE_LINE&resolved=false&sinceLeakPeriod=true&types=CODE_SMELL&ps=100&organization=default-organization&facets=severities%2Ctypes&additionalFields=_all"
$vulnerabilitiesUrl = "https://sonar.transunion.com/api/issues/search?componentKeys=" + $sqprojectname + "&s=FILE_LINE&resolved=false&sinceLeakPeriod=true&types=VULNERABILITY&ps=100&organization=default-organization&facets=severities%2CsonarsourceSecurity%2Ctypes&additionalFields=_all"
$dldUrl = "https://sonar.transunion.com/api/measures/component?component=" + $sqprojectname+ "&metricKeys=new_duplicated_lines_density"
$coverageUrl = "https://sonar.transunion.com/api/measures/component?component=" + $sqprojectname + "&metricKeys=new_coverage"
...
foreach ($item in $history)
{
  ...
  foreach($val in $item.measures)
  {
    ...
    if ($val.metric -eq  "bugs")
    {
        ...
        Write-Host "Bugs :" $val.history.value[-1]
        Write-Host "New Bugs :" $newBugs.total        
        $Bugs = $val.history.value[-1] 
        $newBugsCount= $newBugs.total
    ...
    if ($val.metric -eq  "vulnerabilities")
    {
        ...
        Write-Host "Vulnerabilities :" $val.history.value[-1]
        Write-Host "New Vulnerabilities :" $newVulnerabilities.total
        $Vulnerabilities = $val.history.value[-1]
        $newVulnerabilitiesCount = $newVulnerabilities.total
    }
...
switch ($sonarqubeThreshold)
        {
            ...
            MODERATE 
            {
              ...
              if($Bugs -gt 3 -OR $newBugsCount -gt 3 -OR  $Vulnerabilities -gt 0 -OR  $newVulnerabilitiesCount -gt 0)
              {
                  throw 'Code did not meet SonarQube scan standards, failing this pipeline...'; exit 1;
              }
              Break
