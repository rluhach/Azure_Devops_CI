...
if (!($value))
{
    $value = "cvs, .svn, .hg , .git, .bzr, bin , obj,  backup, .idea, node_modules"
}
...
$value = $parsedYAML.build.branches.$branch.cleanupOnDevEC2
Write-Host "##vso[task.setvariable variable=pipeline.cleanupOnDevEC2;isOutput=true]$value"

$value = $parsedYAML.build.branches.$branch.cleanupOnIntStage
Write-Host "##vso[task.setvariable variable=pipeline.cleanupOnIntEC2;isOutput=true]$value"
...
$value = $parsedYAML.service.cleanupProject
Write-Host "##vso[task.setvariable variable=service.cleanupProject;isOutput=true]$value"