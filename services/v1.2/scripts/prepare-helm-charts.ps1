...
#Get-ChildItem
#Write-Host "$parameters.serviceConfig serviceconfigfile"
#Write-Host "$args[2] verify"
$serviceconfig = [IO.File]::ReadAllText(".\user-service-repo\" + $args[2])
...
$serviceMap=@{}
$path=".\user-service-repo\src\**\**\"+ $args[3] +"\appsettings.json"
Write-Host $path
$appsettingJson = Get-Content $path | Out-String | ConvertFrom-Json
Write-Host '##########################################'
